...
from .permissions import (IsEditor, IsJournalist, IsReader,
...
def article_approve(request, article_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request, 
            f'Article approved! Subscribers have been notified.'
        )
        return redirect('news:article_detail', article_id=article.id)
...
def subscribe_journalist(request, journalist_id):
    ...
    journalist = get_object_or_404(
        User, id=journalist_id, role='journalist'
    )
...
def newsletter_create(request):
    ...
    if request.method == 'POST':
        ...
        return redirect('news:newsletter_detail', 
